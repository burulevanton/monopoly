...
void Purchased::printUnMortgageInfo(Player* player[], int playerNum) {
	cout << this->getName() << "(стоимость выкупа: " << this->costOfMortage(player, playerNum)*1.1 << ", арендная стоимость: " << this->mRent[numOfUpgrades] <<",ходов в залоге:" << numOfTurns << ")" << endl;
}
...
void Purchased::doTurn(Player* player[], int playerNum, queue<int> &q1, queue<int> &q2, int numOfPlayers) {
	...
	if (mOwner == playerNum) {
		cout << player[playerNum]->getName() << " попадает на своё поле <" << this->getName()<<">" << endl;
	...
	if (mOwner < 0) {
		...
		case 1:
			if (player[playerNum]->getBalance() >= mCost) {
				...
				cout << player[playerNum]->getName() << " покупает <" << this->getName() << "> за " << mCost << " Р " << endl;
				mOwner = playerNum;
		...
		if (isMortgage == false) {
			...
			cout << player[playerNum]->getName() << " платит " << toPay << " Р за аренду " << endl;
		}
		else {
